...
def main():
    ...
    if data_args.task_name is not None:
        ...
        is_regression = datasets["train"].features["label"].dtype in ["float32", "float64"]  # 如果自己的数据集的标签是浮点数那就是回归任务
        if is_regression:
            num_labels = 1
        else:
    ...
    config = AutoConfig.from_pretrained(
        ...
        apply_lora=model_args.apply_lora,  # 是否用lora
        lora_type=model_args.lora_type,  # 论文中用的svd
        lora_module=model_args.lora_module,
        lora_alpha=model_args.lora_alpha,
        lora_r=model_args.lora_r,  # 初始r值
        apply_adapter=model_args.apply_adapter,
        adapter_type=model_args.adapter_type,
        adapter_size=model_args.adapter_size,
        reg_loss_wgt=model_args.reg_loss_wgt,  # 对应着论文R(P,Q)
            ...
            if 'deberta' in name or 'roberta' in name or 'bert' in name:
                ...
                for trainable_param in trainable_params:
                    if trainable_param in name:
                        ...
                        sub_num_param = 1
                        for dim in param.shape:  # 假如shape为[768, 4]
                            sub_num_param *= dim  # 那么这里是1*768*4
                        num_param += sub_num_param
                        break
            else:
                param.requires_grad = True  ## 任何不属于主干网络的参数，比如分类头等
    else:  # 全量微调
        for name, param in model.named_parameters():
            sub_num_param = 1
    ...
    if (
        ...
        # label_name_to_id.keys()和label_list顺序可能不一样
    ...
    if training_args.do_train:
        for index in random.sample(range(len(train_dataset)), 3):  # 从range的范围里随机选择3个数
    ...
    def compute_metrics(p: EvalPrediction):
        '''
           p = EvalPrediction(
            predictions=model_predictions,  # 模型输出
            label_ids=true_labels          # 真实标签
        )
        '''
